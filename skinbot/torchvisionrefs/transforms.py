...
class Scale(nn.Module):
    ...
    def __init__(
            ...
            target_scale: Tuple[float, float] = (-1, -1),
            interpolation: InterpolationMode = InterpolationMode.BILINEAR,
    ):
        super().__init__()
        if target_scale[0] == -1 or target_scale[1] == -1:
            target_scale = None
        if target_size[0] == -1 or target_size[1] == -1:
            target_size = None
        if target_scale is None and target_size is None:
            raise ValueError("At least one of target_scale or target_size should be specified.")
        if target_size is not None:
            assert target_size[0] > 0 and target_size[1] > 0, "target size must be positive."
            target_scale = None
        else:
            assert target_scale[0] > 0 and target_scale[0] <= target_scale[1], "target scale must be positive."
            target_size = None
    ...
    def forward(
        ...
        if self.target_size is not None:
            # r = min(self.target_size[1] / orig_height, self.target_size[0] / orig_width)
            new_width = self.target_scale[0]  # int(orig_width * r)
            new_height = self.target_scale[1]  # int(orig_height * r)
            ...
            r_w = self.target_scale[0]
            r_h = self.target_scale[1]
            new_width = int(orig_width * r_w)
            new_height = int(orig_height * r_h)
